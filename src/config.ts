...
const DEFAULT_MEDIUM_MODEL = 'llama.cpp/Nex-N2-mini';
const DEFAULT_HEAVY_MODEL = 'llama.cpp/Nex-N2-mini';
...
function validateTier(obj: unknown, name: string): void {
  if (obj !== undefined) {
    if (typeof obj !== 'object' || obj === null) throw new Error(`${name} must be an object`);
    const t = obj as Record<string, unknown>;
    if (t.model !== undefined && typeof t.model !== 'string') throw new Error(`${name}.model must be a string`);
  }
}
...
function normalizeConfig(config: unknown): ComposeConfig {
  ...
  return {
    ...
    'general-medium': { model: cfg['general-medium']?.model ?? DEFAULT_MEDIUM_MODEL },
    'general-heavy': { model: cfg['general-heavy']?.model ?? DEFAULT_HEAVY_MODEL },
  };
}
...
export function createExploreAgent(input: { agent?: Record<string, unknown> }, cfg: ComposeConfig): void {
  ...
  input.agent.explore = {
    ...
    permission: { read: 'allow', glob: 'allow', grep: 'allow', bash: 'allow', edit: 'deny', write: 'deny' },
  };
}

export function createGeneralMediumAgent(input: { agent?: Record<string, unknown> }, cfg: ComposeConfig): void {
  if (!input.agent) input.agent = {};
  input.agent['general-medium'] = {
    model: cfg['general-medium']?.model ?? DEFAULT_MEDIUM_MODEL,
    mode: 'subagent',
    description: 'General-purpose worker for medium complexity tasks',
    permission: { read: 'allow', edit: 'allow', write: 'allow', bash: 'allow', glob: 'allow', grep: 'allow' },
  };
}

export function createGeneralHeavyAgent(input: { agent?: Record<string, unknown> }, cfg: ComposeConfig): void {
  if (!input.agent) input.agent = {};
  input.agent['general-heavy'] = {
    model: cfg['general-heavy']?.model ?? DEFAULT_HEAVY_MODEL,
    mode: 'subagent',
    description: 'General-purpose worker for high complexity tasks — architecture, deep debug, optimization',
    permission: { read: 'allow', edit: 'allow', write: 'allow', bash: 'allow', glob: 'allow', grep: 'allow' },
  };
}
