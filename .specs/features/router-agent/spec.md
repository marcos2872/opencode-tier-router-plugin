...
3. WHEN subagentes são criados THEN @fast, @medium e @heavy SHALL ter `systemPrompt` próprio definindo identidade de tier e regras fixas.
4. AC-004a: Subagentes @fast/@medium/@heavy são criados com `systemPrompt` próprio que define identidade do tier e regras fixas.
5. AC-004b: System prompts dos subagentes são configuráveis via `tiers.json` com os campos `fast.systemPrompt`, `medium.systemPrompt` e `heavy.systemPrompt`, com fallback para prompts padrão embutidos no plugin.
6. WHEN o Router chama `task()` para delegar THEN o subagente correspondente SHALL executar a tarefa usando suas ferramentas permitidas.

**Independent Test**: Validar que o hook `config` retorna o Router bloqueado e os três subagentes com permissões allow/mode subagent e systemPrompt próprio.
...
6. AC-005a: Ao delegar via `task()`, o Router inclui contexto + instrução do usuário no prompt do subagente, mas não inclui as regras fixas do subagente porque elas já estão no `systemPrompt`.

**Independent Test**: Testar que a estrutura de hooks retornada não inclui nenhum hook além de `config` e que a task delegada recebe instrução + contexto, sem duplicar regras fixas.
...
5. AC-004c: Usuário pode customizar os system prompts dos subagentes em `tiers.json` com `fast.systemPrompt`, `medium.systemPrompt` e `heavy.systemPrompt`.

**Independent Test**: Comparar os system prompts retornados nos subagentes com os valores atuais de `fast.systemPrompt`, `medium.systemPrompt` e `heavy.systemPrompt`.
...
- WHEN `fast.systemPrompt`, `medium.systemPrompt` ou `heavy.systemPrompt` não é string THEN a validação SHALL retornar erro.
...
| RF-016 | Subagentes @fast/@medium/@heavy possuem `systemPrompt` próprio | Design/Implementation/Verification | Pending |
| RF-017 | Prompt da task inclui instrução + contexto sem repetir regras fixas | Design/Implementation/Verification | Pending |
| RF-018 | `fast.systemPrompt`, `medium.systemPrompt` e `heavy.systemPrompt` são configuráveis via `tiers.json` | Design/Implementation/Verification | Pending |

**Coverage:** 18 total, 18 mapped to implementation/design/test work, 0 unmapped ⚠
...
5. AC-004a: Dado `tiers.json` válido, subagentes @fast/@medium/@heavy são criados com `systemPrompt` próprio que define identidade do tier e regras fixas.
6. AC-004b: Dado `tiers.json` com `fast.systemPrompt`, `medium.systemPrompt` ou `heavy.systemPrompt`, o plugin usa esses prompts; sem eles, usa prompts padrão embutidos.
7. AC-005: Nenhum hook além de `config` é registrado.
8. AC-005a: Ao delegar via `task()`, o prompt do subagente inclui instrução do usuário e contexto, sem repetir regras fixas do `systemPrompt`.
...
- [ ] `fast.systemPrompt`, `medium.systemPrompt` e `heavy.systemPrompt` são configuráveis via `tiers.json`, com fallback para prompts padrão.
- [ ] Subagentes continuam disponíveis, com ferramentas liberadas, modo `subagent` e system prompts próprios.
- [ ] O prompt da task inclui instrução + contexto sem repetir regras fixas do subagente.
- [ ] Testes cobrem a criação do Router, subagentes, hooks registrados, prompts delegados e validação de config.
